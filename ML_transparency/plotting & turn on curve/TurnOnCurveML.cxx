...
void TurnOnCurveML() {
    ...
    //float conteggi_tot = nbin * transparency_size;
    float conteggi_tot =0;
...
//         std::cout << ibin << "/" << nbin << std::endl;
        ...
        for (int i = 0; i < transparency_size ; i++) {
            ...
            sum_weight +=1; //lumi_inst_0/Lumi_inst[i];
            ...
            if (value_smeared > threshold) {
                ...
                if (value > 30){
                    conteggi_sopra_rosso = conteggi_sopra_rosso + 1;
                }

                if (value < 30){
                    conteggi_sotto_rosso = conteggi_sotto_rosso + 1;
                }

            }

            //conteggio degli eventi persi nel caso reale e nel caso corretto

            if (value_smeared_real < threshold) {
                //h_correction->Fill(value);//,lumi_inst_0/Lumi_inst[i]);

                eventi_persi_real = eventi_persi_real+1;
            }

            if (value_smeared < threshold) {
                //h_correction->Fill(value);//,lumi_inst_0/Lumi_inst[i]);

                eventi_persi_corrected = eventi_persi_corrected+1;
            }
            ...
            if (value_smeared_real > threshold) {
                ...
                if (value > 30){
                    conteggi_sopra_blu = conteggi_sopra_blu + 1;
                }

                if (value < 30){
                    conteggi_sotto_blu = conteggi_sotto_blu + 1;
                }
            }
    ...
    h_correction->SetLineColor(kOrange);
    h_correction->SetStats(0);
    h_correction->SetFillColor(kOrange);
    ...
    h_real->SetLineColor(kCyan);
    h_real->SetStats(0);
    h_real->SetFillColor(kCyan);
    ...
    legend->SetHeader("trigger efficiency i-Ring 18 ");
    legend->Draw();
    ...
    real_point->SetMarkerColor(kCyan);
    real_point->SetLineColor(kCyan);
    ...
    correction_point->SetMarkerColor(kOrange);
    correction_point->SetLineColor(kOrange);
    ...
    p_legend->SetHeader("test dataset i-Ring 18");
    p_legend->Draw();
